...
function playRecording(e) {
  ...
  if (audio && audio.tagName === 'AUDIO') {
    ...
    prompts.forEach(entry => {
      if(audio.currentSrc.indexOf(entry.file) !== -1) {
        picture.src = entry.file.replace("-gen.mp3", ".png") 
      }
    })
...
function saveRecording() {
  if(file.files.length > 0) {
     ...
     if(ext !== "png") {
       alert("Not supported type " + ext)  
       return
     }

     if(image.size > 500000) {
      alert(`Please select an image with smaller image size. The selected image size is ${(image.size/1024/1024).toFixed(2)} MB.`)
      return
     }
  } else {
